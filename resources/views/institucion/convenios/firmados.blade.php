...
@section('title', 'Convenios Firmados')
    ...
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-gray-600 mt-1">Historial de convenios firmados por la institución</p>
            </div>
            <a href="{{ route('institucion.convenios.index') }}" class="bg-gradient-to-r from-indigo-500 to-blue-500 hover:from-indigo-600 hover:to-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-all duration-300 flex items-center">
    ...
    <div class="grid grid-cols-1 gap-6">
        @forelse($convenios as $convenio)
            <div class="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100">
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="w-full md:w-1/4 bg-gradient-to-br from-blue-50 to-blue-100 p-6">
                        <div class="flex items-center">
                            ...
                            @else
                                <div class="h-12 w-12 rounded-full mr-4 bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white font-bold text-lg border-2 border-white shadow-md">
                        ...
                        <div class="mt-4 text-sm text-gray-700">
                            <div class="flex items-center mb-2">
                                ...
                                <span>Firmado: <strong>{{ $convenio->fecha_firma_institucion ? $convenio->fecha_firma_institucion->format('d/m/Y') : 'N/A' }}</strong></span>
                            </div>
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                </svg>
                                <span>Estado: <strong class="text-green-600">Firmado</strong></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-full md:w-2/4 p-6 border-t md:border-t-0 md:border-l md:border-r border-gray-100">
                        ...
                        <div class="mb-4">
                            <p class="text-gray-700"><span class="font-medium">Empresa:</span> {{ $convenio->oferta->empresa->nombre ?? ($convenio->empresa ? $convenio->empresa->nombre : 'N/A') }}</p>
                            <p class="text-gray-700 mt-1"><span class="font-medium">Oferta:</span> {{ $convenio->oferta->titulo ?? 'Convenio directo' }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-blue-50 rounded-lg p-3">
                                <p class="text-xs text-blue-600 font-medium">FIRMADO POR</p>
                                <p class="text-lg text-gray-800 font-semibold">{{ $convenio->firmadoPorInstitucion ? $convenio->firmadoPorInstitucion->nombre : 'N/A' }}</p>
                            </div>
                            <div class="bg-blue-50 rounded-lg p-3">
                                <p class="text-xs text-blue-600 font-medium">FECHA DE FIRMA</p>
                                <p class="text-lg text-gray-800 font-semibold">{{ $convenio->fecha_firma_institucion ? $convenio->fecha_firma_institucion->format('d/m/Y') : 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-full md:w-1/4 p-6 bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col justify-between">
                        ...
                        <div class="mt-4 p-3 bg-green-50 border border-green-100 rounded-lg">
                            <div class="flex items-center text-green-800 mb-2">
                                ...
                                <span class="font-bold">Convenio Firmado</span>
                            </div>
                            <p class="text-sm text-green-700">Este convenio ya ha sido firmado y está activo para el periodo de prácticas establecido.</p>
                        </div>
                    </div>
                </div>
            </div>
        @empty
            <div class="bg-white rounded-xl shadow-md p-8 text-center border border-gray-100">
                ...
                <h3 class="text-xl font-semibold text-gray-700 mb-2">No hay convenios firmados</h3>
                <p class="text-gray-500 max-w-md mx-auto">Aquí aparecerán los convenios una vez que sean firmados por la institución.</p>
            </div>
        @endforelse
    </div>
