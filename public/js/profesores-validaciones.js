document.addEventListener('DOMContentLoaded', function() {
    ...
    const validaciones = {
        ...
        institucion_id: {
            validar: (valor) => valor && valor !== '',
            mensaje: 'Debes seleccionar una institución'
        },
        especialidad: {
            validar: (valor) => {
                // Opcional, pero si se proporciona debe tener al menos 3 caracteres
                if (valor.trim() === '') return true;
        ...
        num_registro: {
            ...
            mensaje: 'El número de registro debe tener al menos 3 caracteres'
        },
        titulo_academico: {
            validar: (valor) => {
                // Opcional, pero si se proporciona debe tener al menos 3 caracteres
            ...
            mensaje: 'El título académico debe tener al menos 3 caracteres'
        },
        experiencia_anios: {
            validar: (valor) => {
                // Opcional, pero si se proporciona debe ser un número entre 0 y 70
                if (valor.trim() === '') return true;
                const num = parseInt(valor);
                return !isNaN(num) && num >= 0 && num <= 70;
            },
            mensaje: 'La experiencia debe ser un número entre 0 y 70 años'
        },
        sitio_web: {
            validar: (valor) => {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            const camposObligatorios = [
                'nombre', 'email', 'dni', 'institucion_id'
            ];
            ...
            const camposOpcionales = [
                ...
                'fecha_nacimiento', 'especialidad', 'num_registro', 'titulo_academico',
                'experiencia_anios', 'sitio_web', 'descripcion', 'imagen'
            ];
            ...
            .then(data => {
                if (data.success) {
                    ...
                    window.mostrarMensajeExito(data.message || 'Operación realizada con éxito');
                    
                    // Recargar solo la tabla
                    window.refreshProfesoresTable();
                } else if (data.errors) {
    ...
    // Manejar eventos iniciales para botones de la tabla
    initTableButtons();
